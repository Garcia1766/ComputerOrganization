...
reg[`InstBus] sram1_data_o;
reg[`InstBus] sram2_data_o;
        ...
        if (mem_ce_i == 1'b1) begin
            if (mem_addr_i == 32'hBFD003F8) begin // UART串口
                if (mem_we_i == 1'b1) begin //写串口
                    uart_wrn <= 1'b0;
                    uart_data <= mem_data_i;
                end else begin    //读串口
                    uart_rdn <= 1'b0;
                    mem_data_o <= {24'b0, baseram_bus[7:0]};
                end
...
always_comb begin
    ...
    if (sram_no == 1'b0) begin  //0x8......
        ...
        sram1_we_o      <= ~(sram_ce_o & sram_we_o);
        sram1_oe_o      <= ~(sram_ce_o & (~sram_we_o));
        sram1_addr_o    <= sram_addr_o;
        if ((sram_ce_o == 1'b1) && (sram_we_o == 1'b1)) begin
        ...
        sram2_we_o      <= ~(sram_ce_o & sram_we_o);
        sram2_oe_o      <= ~(sram_ce_o & (~sram_we_o));
...
assign sram_data_i = sram_no ? extram_bus : baseram_bus;
assign extram_bus = sram2_data_o;
assign baseram_bus = uart_wrn ? sram1_data_o : uart_data;
